...
def take_user_input(input_type, min_value, max_value):
    """
    Runs a while loop to collect a valid integer from the user via the terminal, which must be one number. 
...
def play_again():
    """
    Runs a while loop to collect a valid string from the user via the terminal, which must be one character y or n. 
    The loop will repeatedly request data, until it is valid. If answer is y it will clear terminal and call main function which will start game again.
    """
    while True:
        try:
